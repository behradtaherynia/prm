...
//    private string $format;
    ...
    /**
     * model\SmartDate constructor.
     * @param $input :: مقدار ورودی که میتواند هم تاریخ شمسی و هم میلادی به صورت رشته باشد و هم تایم استمپ
     * @param $inputType :: 'timestamp','string : 1399-02-12'
     * @param $dateType :: 'gregorian','jalali','hijri'
     */
    public function __construct($input = '', string $inputType = 'timestamp', string $dateType = 'gregorian')
    {
        if ($input == '') {
            $this->timestamp = time();
        } else {
...
// region Class Gregorian Pro Functions::
     ...
     */
    public function getDateStringGregorian(string $separator = '-'): string
    {
     ...
     */
    public function getDateArrayGregorian(): array
    {
        return array(
     ...
     */
    public function getThisWeekFirstDayGregorian(): SmartDate
    {
        $timeStamp = strtotime($this->getDateStringGregorian() . ' -' . $this->getGregorianWeekDayNumber() . ' days');
    ...
    public static function GregorianWeek(int $gotoWeek = 0): array
    {
        ...
        $selectedDateWeekFirstDay = $selectedDate->getThisWeekFirstDayGregorian();
...
// region Class Jalali Pro Functions::
    ...
    public function getDateStringJalali(string $separator = '-'): string
    {
     ...
     */
    public function getDisplayDateJalali(bool $showTime = false): string
    {
        if (!$showTime)
            return $this->getJalaliWeekDay() . ' ' . $this->getJalaliDay() . ' ' . $this->getJalaliMonthName() . ' ' . $this->getJalaliYear();
        else
            return '<strong>' . $this->getJalaliWeekDay() . ' ' . $this->getJalaliDay() . ' ' . $this->getJalaliMonthName() . ' ' . $this->getJalaliYear() . '</strong>' . ' (ساعت ' . $this->getTime() . ')';
    ...
    public function getDateArrayJalali(): array
    {
        return array(
            $this->getJalaliYear(),
            $this->getJalaliMonth(),
     ...
     */
    public function getThisYearFirstDayJalali(): SmartDate
    {
     ...
     */
    public function getThisYearLastDayJalali(): SmartDate
    {
        $thisLastMonth = new SmartDate($this->getJalaliYear() . '-12-1', 'string', 'jalali');
        return $thisLastMonth->getThisMonthLastDayJalali();
    }

    /**
     * @return SmartDate آخرین روز شمسی این ماه
     */
    public function getThisMonthLastDayJalali(): SmartDate
    {
     ...
     */
    public function getThisMonthFirsDayJalali(): SmartDate
    {
        $month = $this->getJalaliMonth();
        $year = $this->getJalaliYear();
     ...
     * @return SmartDate
     */
    public function getThisWeekFirstDayJalali(): SmartDate
    {
        $timeStamp = strtotime($this->getDateStringGregorian() . ' -' . $this->getJalaliWeekDayNumber() . ' days');
        return new SmartDate($timeStamp);
    }
     ...
     * @return SmartDate
     */
    public function getThisWeekLastDayJalali(): SmartDate
    {
        $timeStamp = strtotime($this->getDateStringGregorian() . ' +' . (6 - $this->getJalaliWeekDayNumber()) . ' days');
        return new SmartDate($timeStamp);
    }
     ...
     */
    public function nextMonthJalali(): SmartDate
    {
     ...
     */
    public function previousMonthJalali(): SmartDate
    {
    ...
    /**
     * @param int $gotoWeek :: یک عدد است که به صورت مثبت و منفی می‌باشد و نشان دهنده چند هفته قبل یا بعد می‌باشد. عدد صفر به معنی هفته جاری است که می‌تواند وارد نشود
     * @return SmartDate[] :: model\SmartDate List (one week).
     */
    public static function JalaliWeek(int $gotoWeek = 0): array
    {
        $selectedDate = new SmartDate();
        $selectedDate->move($gotoWeek * 7);
        $selectedDateWeekFirstDay = $selectedDate->getThisWeekFirstDayJalali();
    ...
    public static function JalaliMonth(int $gotoMonth = 0): array
    {
        ...
        $thisMonthLastDayNumber = $dateAfterGo->getThisMonthLastDayJalali()->getJalaliDay();
...
// region Class Hijri Base Functions::
    ...
    {
        return intval(self::GregorianToHijri($this->getDateStringGregorian())[2]);
    }
    ...
    {
        return intval(self::GregorianToHijri($this->getDateStringGregorian())[1]);
    }
    ...
    {
        return intval(self::GregorianToHijri($this->getDateStringGregorian())[0]);
    }
...
// region Class Public Pro Functions::
    ...
    public function move($howManyDays): void
    {
        ...
        $this->timestamp = strtotime($this->getDateStringGregorian() . $howManyDays);
    }
    ...
    public function find(int $howMany, string $goType = 'day'): SmartDate
    {
        ...
        $timestamp = strtotime($this->getDateStringGregorian() . $howMany);
        return new SmartDate($timestamp);
    }

    /**
     * @param $prefix :: جمله پیشوند
     * @param $suffix :: جمله پسوند
     * @return string :: انتشار در ۴ ماه پیش
     */
    public function howLongAgo($prefix = '', $suffix = ''): string
    {
    ...
    {
        return Occasion::GetOccasions($this->getHijriDay(), $this->getHijriMonth(), 2);
    ...
    {
        return Occasion::GetOccasions($this->getJalaliDay(), $this->getJalaliMonth(), 1);
    ...
    {
        return Occasion::GetOccasions($this->getGregorianDay(), $this->getGregorianMonth(), 3);
...
// region Class Public Static Pro Functions::
    ...
    public static function YearRemainingDays(): array
    {
        ...
        $yearLastDay = $currentDay->getThisYearLastDayJalali();
...
// region Class Private Static Functions::
    ...
    {
        list($jy, $jm, $jd) = explode('_', SmartDate::NumberConverter($jy . '_' . $jm . '_' . $jd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
        $jy += 1595;
    ...
    {
        list($gy, $gm, $gd) = explode('_', SmartDate::NumberConverter($gy . '_' . $gm . '_' . $gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    ...
    private static function JalaliDate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        list($j_y, $j_m, $j_d) = SmartDate::GregorianToJalali($date[8], $date[3], $date[2]);
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case 'D':
                    $out .= SmartDate::JalaliDateWords(array('kh' => $date[7]), ' ');
                    break;

                case 'f':
                    $out .= SmartDate::JalaliDateWords(array('ff' => $j_m), ' ');
                    break;

                case 'F':
                    $out .= SmartDate::JalaliDateWords(array('mm' => $j_m), ' ');
                    break;
                ...
                case 'J':
                    $out .= SmartDate::JalaliDateWords(array('rr' => $j_d), ' ');
                    break;
                ...
                case 'l':
                    $out .= SmartDate::JalaliDateWords(array('rh' => $date[7]), ' ');
                    break;
                ...
                case 'M':
                    $out .= SmartDate::JalaliDateWords(array('km' => $j_m), ' ');
                    break;
                ...
                case 'p':
                    $out .= SmartDate::JalaliDateWords(array('mb' => $j_m), ' ');
                    break;
                ...
                case 'q':
                    $out .= SmartDate::JalaliDateWords(array('sh' => $j_y), ' ');
                    break;
                ...
                case 'r':
                    $key = SmartDate::JalaliDateWords(array('rh' => $date[7], 'mm' => $j_m));
                ...
                case 'v':
                    $out .= SmartDate::JalaliDateWords(array('ss' => ($j_y % 100)), ' ');
                    break;

                case 'V':
                    $out .= SmartDate::JalaliDateWords(array('ss' => $j_y), ' ');
                    break;
